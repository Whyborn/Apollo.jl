...
function Domain(lons, lats, land_area_fraction{T, 3}, mapping; lon_bnds=nothing, lat_bnds=nothing, mask=nothing, normalise=false) where {T}
    ...
    if (isnothing(lon_bnds))
        lon_bnds = infer_bounds(lons)
    else
        # If not, then make sure the supplied bounds are valid
        check_bounds_valid(lons, lon_bnds)
    end

    if (isnothing(lat_bnds))
        lat_bnds = infer_bounds(lats)
    else
...
"""
Infer the cell bounds, based on the list of cell centre locations passed.
"""
function infer_bounds(points::Vector{T}) where {T}
